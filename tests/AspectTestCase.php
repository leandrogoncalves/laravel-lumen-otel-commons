...
class AspectTestCase extends \PHPUnit\Framework\TestCase
{
    ...
    protected function registerConfigure()
    {
        ...
        $this->app['config']->set(
            "picpay-laravel-aop",
            $filesystem->getRequire(__DIR__ . '/config/picpay-laravel-aop.php')
        );
        $this->app['config']->set(
            "database",
    ...
    protected function createApplicationContainer()
    {
        ...
        $this->registerCache();
        $annotationConfiguration = new \Picpay\LaravelAspect\AnnotationConfiguration(
            $this->app['config']->get('picpay-laravel-aop.annotation')
        );
        $annotationConfiguration->ignoredAnnotations();
        $this->app->singleton('aspect.manager', function ($app) {
            return new \Picpay\LaravelAspect\AspectManager($app);
        });
        $this->app->bind(
